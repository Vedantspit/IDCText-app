...
$tablePrefix = "<h3>SESSION-WISE SUMMARY</h3><table><tr><th colspan='1'>Study ID</th><th colspan='1'>UserID ID</th><th colspan='1'>Keyboard</th><th colspan='1'>Session #</th><th colspan='1'>Error rate without any exceptions</th><th colspan='1'>Error rate with unicode exceptions</th><th colspan='1'>Error rate with unicode and Keyboard exceptions </th><th colspan='1'>Average Speed(cpm)</th><th colspan='1'>Corrected Error Rate1 (without unicode and keyboard exceptions)</th><th colspan='1'>Uncorrected Error rate1(without unicode and keyboard exceptions)</th><th colspan='1'>Total Error rate1(without unicode and keyboard exceptions)</th><th colspan='1'>Corrected Error Rate2(with unicode exceptions)</th><th colspan='1'>Uncorrected Error rate2(with unicode exceptions)</th><th colspan='1'>Total Error rate2(with unicode exceptions)</th><th colspan='1'>Corrected Error Rate3(**with unicode and keyboard exceptions)</th><th colspan='1'>Uncorrected Error rate3(**with unicode and keyboard exceptions)</th><th colspan='1'>Total Error rate3(**with unicode and keyboard exceptions)</th></tr>";
$tableSuffix = "</table>";
$tableBody = "";
...
while ($row_kb = mysqli_fetch_array($result_distinct_users)) {
 ...
 {
    $query_sess="SELECT DISTINCT sessions from nusers where studyid='$sid'  AND  userid='$user' and keyboard='$keyboard' ORDER BY sessions";
    $result_sess = mysqli_query($conn, $query_sess);

if ($result_sess === false)
    die("Query failed: " . mysqli_error($conn));

$distinct_sess = array();

while ($currsess = mysqli_fetch_array($result_sess)) {
    $distinct_sess[] = $currsess['sessions'];
}
foreach($distinct_sess as $currentSess)
{
    $totalErrorRate1 = 0;
    $totalErrorRate2 = 0;
    $totalErrorRate3 = 0;
    ...
    while ($row = mysqli_fetch_array($result)) 
    {
        ...
        $errorRate1 = ($row[3] / $maxStrLen) * 100;
        $errorRate2 = ($row[4] / $maxStrLen) * 100;
        $errorRate3 = ($row[5] / $maxStrLen) * 100;
    ...
    $tableBody .= "<td colspan='1'>" . $currentSess . "</td>";
